...
function normalizeProviderResult(result) {
  if (Array.isArray(result)) {
    return { streams: result, subtitles: [] };
  }

  if (result && typeof result === 'object') {
    return {
      streams: Array.isArray(result.sources) ? result.sources : [],
...
app.get('/api/streams/:type/:tmdbId', async (req,res) => {
  ...
  try {
    ...
    const results = await Promise.all(selectedProviders.map(async name => {
      ...
      if (!prov || !prov.enabled) return { streams: [], subtitles: [] };
      ...
      try {
        ...
        const normalized = normalizeProviderResult(r);
        console.log(`[api] provider ${name} returned ${normalized.streams.length} streams`);
        return normalized;
      } catch (e) {
    ...
    let streams = results.flatMap(result => result.streams);
    const subtitles = results.flatMap(result => result.subtitles);
    ...
    res.json({ success:true, tmdbId, imdbId, count: streams.length, providerTimings, streams, subtitles });
  } catch (e) {
...
app.get('/api/streams/:provider/:type/:tmdbId', async (req,res) => {
  ...
  try {
    ...
    const imdbId = await resolveImdbId(tmdbType, tmdbId); if (imdbId) metrics.tmdbToImdbLookups++;
    const t0 = Date.now();
    const providerResult = normalizeProviderResult(await prov.fetch({ tmdbId, type, season, episode, imdbId, filters:{} }));
    let streams = providerResult.streams;
    ...
    res.json({ success:true, provider: prov.name, tmdbId, imdbId, count: streams.length, providerTimings, streams, subtitles: providerResult.subtitles });
  } catch (e) {
    metrics.lastError = e.message;
    res.status(500).json({ success:false, error:'INTERNAL_ERROR', message:e.message });
  }

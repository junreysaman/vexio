...
<div class="watch-sidebar">
  ...
  <div style="overflow-y:auto;flex:1;">
    <?php foreach (($episodes ?? []) as $idx => $row): ?>
      ...
      ?>
      <a class="<?= escape(implode(' ', $classes)) ?>" href="<?= escape((string) ($row['watchUrl'] ?? $row['watch_url'] ?? '#')) ?>">
        <div class="esc-thumb c<?= ($idx % 8) + 1 ?>">
          <?php if ($rowPoster !== ''): ?><img src="<?= escape($rowPoster) ?>" alt="" style="width:100%;height:100%;object-fit:cover;"><?php else: ?><div class="esc-ph">S<?= $rowSeason ?> E<?= $rowEpisode ?></div><?php endif; ?>
